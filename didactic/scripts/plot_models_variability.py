...
def plot_embeddings_variability(
    ...
    **scatter_kwargs,
    ...
    Args:
        ...
        **scatter_kwargs: Additional keyword arguments to forward to the call to `seaborn.scatterplot`.
    ...
    # For plotting categorical data, count the number of patients assigned to each label
    cat_attrs_labels_counts = {
        attr: {
            str(attr_label): sum(patient_val == attr_label for patient_val in scatter_kwargs[group_var])
            for attr_label in TABULAR_CAT_ATTR_LABELS[attr] or sorted(set(scatter_kwargs[group_var]))
        }
        for group_var in ("hue", "size", "style")
        if group_var in scatter_kwargs and (attr := scatter_kwargs[group_var]) not in TabularAttribute.numerical_attrs()
    }
    ...
    with sns.axes_style("darkgrid"):
        ...
        scatter = sns.scatterplot(data=data, x="val", y="std", **scatter_kwargs)
