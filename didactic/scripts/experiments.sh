...
# finetune
# tabular data only
CARDIAC_MULTIMODAL_REPR_PATH=/home/local/USHERBROOKE/pain5474/data/didactic/results/multirun/cardiac-multimodal-representation COMET_PROJECT_NAME=didactic-finetune didactic-runner -m hydra/launcher=joblib hydra.launcher.n_jobs=16 +experiment=finetune trainer.enable_progress_bar=False task/data=tab-selec,tab-no-echo-data,tab-all task.contrastive_loss_weight=0,0.2 task.cls_token=False,True task.ordinal_mode=False,True '+trial=range(10)' 'ckpt="/home/local/USHERBROOKE/pain5474/data/didactic/multirun/cardiac-multimodal-representation/pretrain/data=${hydra:runtime.choices.task/data}/encoder=cardinal-ft-transformer,cross_blocks=0,time_series_tokenizer=${hydra:runtime.choices.task/time_series_tokenizer/model}/cls_token=${task.cls_token}/+trial=${trial}.ckpt"' >>$HOME/data/didactic/results/finetune,data=tabular-only.log 2>&1
# tabular + time-series data
CARDIAC_MULTIMODAL_REPR_PATH=/home/local/USHERBROOKE/pain5474/data/didactic/results/multirun/cardiac-multimodal-representation COMET_PROJECT_NAME=didactic-finetune didactic-runner -m hydra/launcher=joblib hydra.launcher.n_jobs=16 +experiment=finetune trainer.enable_progress_bar=False task/data=tab-selec+ts,tab-no-echo-data+ts,tab-all+ts task.contrastive_loss_weight=0,0.2 task/time_series_tokenizer/model=linear-embedding,transformer task.cls_token=False,True task.ordinal_mode=False,True '+trial=range(10)' 'ckpt="/home/local/USHERBROOKE/pain5474/data/didactic/multirun/cardiac-multimodal-representation/pretrain/data=${hydra:runtime.choices.task/data}/encoder=cardinal-ft-transformer,cross_blocks=0,time_series_tokenizer=${hydra:runtime.choices.task/time_series_tokenizer/model}/cls_token=${task.cls_token}/+trial=${trial}.ckpt"' >>$HOME/data/didactic/results/finetune,data=tabular+ts.log 2>&1
