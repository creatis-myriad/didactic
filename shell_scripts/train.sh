...
# xtab
didactic-runner -m hydra/launcher=joblib hydra.launcher.n_jobs=5 +experiment=xtab trainer.enable_progress_bar=False task/data=tab-no-echo-data,tab-no-echo-data+ts,tab-all,tab-all+ts ckpt=null,$HOME/data/models/xtab/iter_2k_patch.ckpt 'split_idx=range(5)' >>$CARDIAC_MULTIMODAL_REPR_PATH/xtab.log 2>&1
...
didactic-runner -m hydra/launcher=joblib hydra.launcher.n_jobs=5 +experiment=xtab trainer.enable_progress_bar=False task/data=tab-no-echo-data+ts task.cls_token=False,True task.ordinal_mode=False,True 'split_idx=range(5)' >>$CARDIAC_MULTIMODAL_REPR_PATH/xtab_latent_representation_ablation_study.log 2>&1
