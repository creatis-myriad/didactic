...
didactic-runner -m hydra/launcher=joblib hydra.launcher.n_jobs=5 +experiment=xtab trainer.enable_progress_bar=False task/data=tab-no-echo-data+ts task.cls_token=False task.sequence_pooling=True task.ordinal_mode=False,True 'split_idx=range(5)' >>$OUTPUT_DIR/xtab_latent_representation_ablation_study.log 2>&1
