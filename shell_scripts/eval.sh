...
for split_idx in $(seq 0 4); do
  ...
  python ~/remote/didactic/didactic/scripts/describe_patients.py --data_roots $CARDINAL_DATA_PATH --views A4C A2C --attributes mean_e_prime la_volume lvm_ind --subsets $(find $OUTPUT_DIR/continuum_param_bins/split_idx=$split_idx -name "*.txt" | sort | tr "\n" " ") --output_dir=$OUTPUT_DIR/split_idx=$split_idx/describe_patients_by_bins >>$OUTPUT_DIR/describe_patients_by_bins.log 2>&1
  ...
  python ~/remote/didactic/vital/vital/data/cardinal/plot_attrs_wrt_groups.py --data_roots $CARDINAL_DATA_PATH --views A4C A2C --groups_txt $(find $OUTPUT_DIR/continuum_param_bins/split_idx=$split_idx -name "*.txt" | sort | tr "\n" " ") '--time_series_plot_kwargs={errorbar:,palette:flare}' --output_dir=$OUTPUT_DIR/split_idx=$split_idx/attrs_wrt_continuum_param >>$OUTPUT_DIR/attrs_wrt_continuum_param.log 2>&1
  ...
  python ~/remote/didactic/didactic/scripts/cardiac_multimodal_representation_plot.py "$MODEL_COMMON_PATH-$split_idx" --data_roots $CARDINAL_DATA_PATH --views A4C A2C --plot_categorical_attrs_dirs $OUTPUT_DIR/split_idx=$split_idx/continuum_param_bins --plot_markers_attrs_dir=$CARDINAL_DATA_PATH/splits/$split_idx '--num_plot_kwargs={palette:flare}' --output_dir=$OUTPUT_DIR/split_idx=$split_idx/2d_embeddings >>$OUTPUT_DIR/2d_embeddings.log 2>&1
